"""
Layer 2 Step 5b DIAGNOSTIC v3 — Census/ABS subset layout discovery
─────────────────────────────────────────────────────────────────
v3 changes from v2:
  - Fixed TypeError: probe_workbook_all_tables() now consistently
    uses 'patterns' as keyword name throughout.

v2 changes from v1:
  - Probes ALL tables (Table 1, 2, 3) in BOTH workbooks
  - Confirmed both workbooks are TIME SERIES (2011-2025)
  - Search patterns broadened

READ-ONLY. No DB mutations. No file writes outside recon/.

Outputs:
  - Console summary
  - recon/layer2_step5b_diag_v3.md
...
OUT_MD = OUT_DIR / "layer2_step5b_diag_v3.md"
...
def probe_table(wb_path: Path, sheet_name: str,
                patterns: list[str], label: str) -> dict:
    ...
    if len(rows) >= 4:
        ...
        print(f"    Title row 4: "
              f"{str(title_cell)[:120] if title_cell else '(empty)'}")

    print(f"    Row 6 cols 0-3: "
          f"{[str(c)[:40] if c else '·' for c in rows[5][:4]]}")
    print(f"    Row 7 cols 0-3: "
          f"{[str(c)[:40] if c else '·' for c in rows[6][:4]]}")
    print(f"    Total cols: {n_cols}")

    matches = find_columns_matching(rows, n_cols, patterns)
    ...
    for c_idx, header in matches[:20]:
        ...
        if samples:
            ...
            print(f"             samples: "
                  f"{[round(s, 2) for s in samples[:5]]}")
...
def main() -> int:
    ...
    column_map = {
        "schema_note": (
            "Both workbooks are TIME SERIES 2011-2025. Schema target: "
            "long format (sa2_code, year, metric_name, value)."
        ),
        "income": income_info,
        ...
        "next_step": (
            ...
            "median income and ONE column for female LFP."
        ),
    }
    OUT_JSON.write_text(json.dumps(column_map, indent=2, default=str),
                        encoding="utf-8")

    findings = ["# Layer 2 Step 5b Diagnostic v3 — All Tables", ""]
