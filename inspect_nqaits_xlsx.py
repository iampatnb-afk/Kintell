...
xlsx = Path("abs_data/NQAITS Quarterly Data Splits (Q3 2013 - Q4 2025).XLSX")
...
Path("nqaits_xlsx_inspect.json").write_text(json.dumps(summary, indent=2), encoding="utf-8")
print(f"Sheets: {list(summary.keys())}")
for name, info in summary.items():
    print(f"  {name}: {info['row_count']} rows, {len(info['headers'])} columns")
print("wrote nqaits_xlsx_inspect.json")
