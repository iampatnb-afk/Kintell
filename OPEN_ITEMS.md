...
**Closure (2026-05-10 PM session 3).** Root cause was postcode-vs-polygon SA2 attribution mismatch — `build_sa2_history.py` v2 used `postcode_to_sa2_concordance.csv` (one-postcode→one-SA2), but the centre page uses polygon-derived `services.sa2_code` (DEC-70 backfill, 99.89% coverage). The two systems disagreed for 1,043 SA2s. Failure modes verified empirically: cross-state name collisions (PC 6102 → QLD "Bentley Park" not WA "Bentley-Wilson"), multi-SA2 postcodes (PC 6107 → neighbour SA2), missing-from-concordance postcodes (PC 0822 NT — empty entry). Fix: switched primary attribution to Service Approval Number → `services.sa2_code` lookup; postcode-concordance retained as fallback for closed/de-registered services and pre-Q12022 NQAITS rows. **Coverage 1,267 → 2,309 SA2s (+1,042). 0 residual gap** — every services-anchored SA2 now appears in `docs/sa2_history.json`. All 4 verifying SA2s render: Bayswater Vic ✓, Bondi Junction NSW ✓, Bentley-Wilson WA ✓ (6 LDC + 4 OSHC services, 448 places Q4 2024), East Arnhem NT ✓ (was the polygon-derived name; "Outback NT" was concordance-shorthand). 15 fallback-attributed SA2s (down from 16 phantoms in v2) — pre-Q12022 closed centres mapped via concordance fallback. STD-08 backups at `data/sa2_history.json.backup_pre_oi_new_21_20260510_160747.json` + `docs/sa2_history.json.pre_oi_new_21_20260510_160747.json`. File size 13.8 → 19.3 MB. audit_log 174 → 175.
