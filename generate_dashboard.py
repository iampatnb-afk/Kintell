...
<div class="panel active" id="panel-industry">
    ...
    <div class="stat-grid">
        ...
        <div class="stat-card">
            <div class="stat-label">Operators Tracked</div>
            <div class="stat-value">{len(operators):,}</div>
            <div class="stat-sub">{sum(1 for o in operators if o.get('priority_tier') in ('hot','warm'))} hot+warm targets</div>
            <div id="growth-fp" style="margin-top:8px;font-size:11px;color:var(--muted)">Loading...</div>
        </div>
        <div class="stat-card">
            <div class="stat-label">Weekly Update</div>
            <div class="stat-value" style="font-size:16px;">{today}</div>
            <div class="stat-sub">Next: Tuesday 10am</div>
            <div id="growth-hist" style="margin-top:8px;font-size:11px;color:var(--muted)">Loading...</div>
        </div>
    </div>
    ...
    <div class="section" style="margin-bottom:24px">
        ...
        <div style="display:grid;grid-template-columns:1fr 1fr;gap:24px">
            ...
            <div>
                <div style="font-size:11px;text-transform:uppercase;letter-spacing:.08em;color:var(--muted);margin-bottom:8px">For-Profit vs NFP (share of all services)</div>
                <canvas id="chart-mgmt" height="120"></canvas>
            </div>
            <div>
...
function buildStatGrowths() {{
    ...
    const fEl = document.getElementById('growth-fp');
    ...
    if (fEl) {{
        ...
        fEl.innerHTML = 'For-profit share: ' + growthLine(fpPct);
    }}
...
function buildTrendCharts() {{
    ...
    // 5. For-profit vs NFP absolute
    chartInstances['mgmt'] = new Chart(document.getElementById('chart-mgmt'), {{
        type: 'line', options: chartOpts,
        data: {{ labels, datasets: [
            makeDataset('For-Profit', histData.forprofit, '#3d7eff'),
            makeDataset('NFP',        histData.nfp,       '#8890a8'),
        ] }}
    }});
